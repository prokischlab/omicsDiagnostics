#'---
#' title: Figure 2b EPG5 case example with protein only
#' author: smirnovd
#' wb:
#'  input:
#'  - config: 'src/config.R'
...
#'  output:
#'  - fig_human: '`sm config["FIGURE_DIR"] + "/Fig2_b_human.pdf"`'
#'  - fig: '`sm config["FIGURE_DIR"] + "/Fig2_b.pdf"`'
#' output: 
#'   html_document:
#'    code_folding: hide
#'    code_download: TRUE
...
fig_human <- gganatogram(data=  patients[SAMPLE_ID == "OM06865"], 
                           fillOutline='white', organism='human', 
                           sex= unique(patients[SAMPLE_ID == "OM06865"]$sex), 
                           fill="colour")+ 
  theme_void() + 
...
pdf(snakemake@output$fig_human, # "/s/project/mitoMultiOmics/multiOMICs_integration/Figures/Fig2_b_human.pdf",  
...
samp <- rp[ SAMPLE_ID == "OM06865" & outlier_class != "non_outlier"]
fig <- ggplot(samp[gene_detected == "RNA and protein detected" ], aes(RNA_ZSCORE, PROTEIN_ZSCORE)) +
  ...
  coord_fixed( xlim = c( -8, 7 ),  ylim = c( -8,  7 )) +  
  theme_bw() +
...
pdf(snakemake@output$fig, # "/s/project/mitoMultiOmics/multiOMICs_integration/Figures/Fig2_b.pdf",  
...
autoplot(edb, ~ gene_name == "EPG5", stat = "reduce" ,  names.expr="EPG5") + 
  theme_void()  
 # geom_vline( xintercept =  173800770 , colour = "darkblue")
dev.off()

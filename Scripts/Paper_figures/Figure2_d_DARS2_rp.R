...
#'  output:
#'  - fig_human: '`sm config["FIGURE_DIR"] + "/Fig2_d_human.pdf"`'
...
fig_human <- gganatogram(data=  patients[SAMPLE_ID == "OM75740"], 
                           fillOutline='white', organism='human', 
                           sex= unique(patients[SAMPLE_ID == "OM75740"]$sex), 
                           fill="colour")+ 
  theme_void() + 
...
samp <- rp[ SAMPLE_ID == "OM75740" & outlier_class != "non_outlier"]
...
autoplot(edb, ~ gene_name == "DARS2", stat = "reduce" ,  names.expr="DARS2") + 
  theme_void() +  

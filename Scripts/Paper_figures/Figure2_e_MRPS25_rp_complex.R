#'---
#' title: Figure 2e  MRPS25 case example protein complex
#' author: smirnovd
#' wb:
#'  input:
#'  - config: 'src/config.R'
...
#'  output:
#'  - fig_human: '`sm config["FIGURE_DIR"] + "/Fig2_e_human.pdf"`'
#'  - fig: '`sm config["FIGURE_DIR"] + "/Fig2_e.pdf"`'
#' output: 
#'   html_document:
#'    code_folding: hide
#'    code_download: TRUE
...
fig_human <- gganatogram(data=  patients[SAMPLE_ID == "OM36526"], fillOutline='white', organism='human', 
                           sex= unique(patients[SAMPLE_ID == "OM36526"]$sex), fill="colour")+ 
  theme_void() + 
  theme(plot.title = element_text(margin = NULL,face="bold", size = 9, hjust = 0.5),
        plot.margin = margin(0, 0, 0, 0, "cm"))
fig_human

pdf(snakemake@output$fig_human, # "/s/project/mitoMultiOmics/multiOMICs_integration/Figures/Fig2_e_human.pdf",  
    width = 3, height =5,  useDingbats=FALSE )
print(fig_human) 
dev.off()


#subset for patient and add WES candidate VPS11
samp <- rp[ SAMPLE_ID == "OM36526"  & (outlier_class != "non_outlier" | geneID == "VPS11")] 
...
autoplot(edb, ~ gene_name == "MRPS25", stat = "reduce" ,  names.expr="MRPS25") + 
